...
type KubeInformerFactory interface {
	...
	USBDevicesConfig() cache.SharedIndexInformer
}
...
func (f *kubeInformerFactory) USBDevicesConfig() cache.SharedIndexInformer {
	return f.getInformer("usbDevicesConfigInformer", func() cache.SharedIndexInformer {
		lw := cache.NewListWatchFromClient(f.clientSet.GeneratedKubeVirtClient().UsbV1alpha1().RESTClient(), "usbDevicesConfigs", k8sv1.NamespaceAll, fields.Everything())
		return cache.NewSharedIndexInformer(lw, &v1alpha1.USBDevicesConfig{}, f.defaultResync, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
	})
}
